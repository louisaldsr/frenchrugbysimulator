...
export function computeRanking(
  ...
  const ensureTeamRanked = (teamId: string): TeamRanking => {
    if (!table.has(teamId)) {
      ...
      table.set(teamId, createEmptyRanking(teamId, team.initialPoints));
    }
...
function handleWinnerPoints(
  ...
  return {
    ...currentRanking,
    totalMatchs: {
      ...currentRanking.totalMatchs,
      wins: (currentRanking.totalMatchs.wins += 1),
    },
...
function handleLoserPoints(
  ...
  return {
    ...
    totalMatchs: {
      ...currentRanking.totalMatchs,
      losses: (currentRanking.totalMatchs.losses += 1),
    },
